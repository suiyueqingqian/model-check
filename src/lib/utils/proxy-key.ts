...
export async function getBuiltInProxyKeyInfo(): Promise<BuiltInProxyKeyInfo> {
  ...
  if (builtInRecord) {
    return {
      ...
      temporaryStopValue: builtInRecord.temporaryStopValue,
      temporaryStopUnit: builtInRecord.temporaryStopUnit,
      unifiedRouteStrategy: builtInRecord.unifiedRouteStrategy,
...
export async function validateProxyKey(apiKey: string): Promise<ValidateKeyResult> {
  ...
  if (!ENV_PROXY_API_KEY) {
    ...
    if (apiKey === autoKey) {
      return {
        valid: true,
        keyRecord: createTransientBuiltInProxyKey(autoKey, "auto"),
